...
class PlayState extends MusicBeatState
{
	...
	var col:Array<FlxColor> = [
		...
		0xFFBD2965, // GF
		0xFF00FF90, // garcello
		0xFF00FF90, // garcellotired
		0xFF01FE90, // garcellodead
		0xFF449465, // garcelloghosty
		0xFF8B77A7, // neo-garcello
		0xFF8B77A7, // neo-garcellotired
		0xFFE30BF7, // neo-garcellodead
		0xFFA121AA, // neo-garcelloghosty
	];
	...
	override public function create()
	{
		...
		{
			case 'nerves':
				songLowercase = 'nerves-neo';
			case 'release':
				songLowercase = 'release-neo';
			case 'headache':
				songLowercase = 'headache-neo';
			case 'fading':
				songLowercase = 'fading-neo';
		}
		...
		switch (songLowercase)
		{
			...
			case 'headache-neo':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/headache/headache-neoDialogue'));
			case 'nerves-neo':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/nerves/nerves-neoDialogue'));
			case 'release-neo':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/release/release-neoDialogue'));
			case 'fading-neo':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/fading/fading-neoDialogue'));
		}
				...
				case 'schoolEvil':
					{
						...
						case 'garAlley':
						{
								...
								var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('gar/garStagebg'));
								bg.antialiasing = true;
								bg.scrollFactor.set(0.7, 0.7);
								bg.active = false;
								add(bg);
		
								var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('gar/garStage'));
						...
						case 'garAlleyDead':
						{
								...
								var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('gar/garStagebgAlt'));
								bg.antialiasing = true;
								bg.scrollFactor.set(0.7, 0.7);
								bg.active = false;
								add(bg);
								...
								smoker.frames = Paths.getSparrowAtlas('gar/garSmoke');
								...
								var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('gar/garStagealt'));
								bgAlley.antialiasing = true;
								bgAlley.scrollFactor.set(0.9, 0.9);
								bgAlley.active = false;
								add(bgAlley);
		
								var corpse:FlxSprite = new FlxSprite(-230, 540).loadGraphic(Paths.image('gar/gardead'));
						...
						case 'garAlleyDip':
						{
								...
								var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('gar/garStagebgRise'));
								bg.antialiasing = true;
								bg.scrollFactor.set(0.7, 0.7);
								bg.active = false;
								add(bg);
		
								var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('gar/garStageRise'));
								bgAlley.antialiasing = true;
								bgAlley.scrollFactor.set(0.9, 0.9);
								bgAlley.active = false;
								add(bgAlley);
		...
		if (curStage == 'garAlleyDead')
			{
				...
				smoke.frames = Paths.getSparrowAtlas('gar/garSmoke');
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				case "winter-horrorland":
					...
					case 'headache':
						var introText:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('gar/garIntroText'));
	...
	function garIntro(?dialogueBox:DialogueBox):Void
		{
			...
			sexycutscene.frames = Paths.getSparrowAtlas('gar/GAR_CUTSCENE');
		...
		function garIntroneo(?dialogueBox:DialogueBox):Void
			{
				var black:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
				black.scrollFactor.set();
				add(black);
				...
				var sexycutscene:FlxSprite = new FlxSprite();
				sexycutscene.antialiasing = true;
				sexycutscene.frames = Paths.getSparrowAtlas('gar/GAR_CUTSCENE_NEO');
				sexycutscene.animation.addByPrefix('video', 'garcutscene', 15, false);
				sexycutscene.setGraphicSize(Std.int(sexycutscene.width * 2));
				sexycutscene.scrollFactor.set();
				sexycutscene.updateHitbox();
				...
				if (SONG.song.toLowerCase() == 'nerves-neo' || SONG.song.toLowerCase() == 'release-neo')
				{
					remove(black);
		
					if (SONG.song.toLowerCase() == 'release-neo')
					{
						add(red);
					}
				}
	...
	override function stepHit()
	{
		...
		if (dad.curCharacter == 'garcelloghosty' && SONG.song.toLowerCase() == 'fading')
			...
			if (dad.curCharacter == 'neo-garcelloghosty' && SONG.song.toLowerCase() == 'fading-neo')
				{
					if (curStep == 256)
					{
